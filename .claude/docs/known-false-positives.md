...
- **Repeated `custom_effect_tooltip` in one option**: `custom_effect_tooltip` is an effect, not a single-valued field, so an option may call it several times and every call renders its own line. The common pairing is one authored `<event_id>.<opt>_tt` summary plus one `{ localization_key = modifies_dynamic_modifier_tt MODIFIER = ... }` block. Deduplicating them deletes a player-visible tooltip and orphans its loc key. Around 300 options across 54 event files use the pattern.
