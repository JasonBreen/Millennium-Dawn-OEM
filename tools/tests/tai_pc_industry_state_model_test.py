...
def test_visible_event_routes_are_distinct_clamped_and_consumed_later():
    ...
    assert event_ids == [
        *(f"TAI_pc_industry_events.{number}" for number in range(1, 16)),
        "TAI_pc_industry_events.90",
    ]
    ...
    anchor = _event_block(events, 90)
    assert "hidden = yes" in anchor
    assert "TAI_pc_industry_initialize_state = yes" in anchor
    assert "TAI_pc_industry_reconstruct_history = yes" in anchor
    assert "country_event =" not in anchor.partition("immediate = {")[2]
