...
def test_initialization_clamps_and_event_surface_are_exact():
    ...
    assert set(events) == set(range(1, 11)) | {90}
    ...
    assert "is_triggered_only = yes" in events[90]
    assert "hidden = yes" in events[90]
    assert "picture =" not in events[90]
