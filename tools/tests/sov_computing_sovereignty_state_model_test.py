...
def test_event_inventory_is_eighteen_visible_plus_hidden_reconstruction():
    ...
    expected_ids = set(range(1, 19)) | {90}
    ...
    hidden = _event_block(events, 90)
    assert "is_triggered_only = yes" in hidden
    assert "SOV_computing_sovereignty_reconstruct_history = yes" in hidden
...
def test_hidden_reconstruction_cannot_race_an_open_visible_milestone():
    events = EVENTS_PATH.read_text(encoding="utf-8")
    hidden_reconstruction = _event_block(events, 90)
    trigger = _named_block(hidden_reconstruction, "trigger")

    assert "SOV_computing_sovereignty_no_event_pending = yes" in trigger
    assert (
        "SOV_computing_sovereignty_reconstruct_history = yes" in hidden_reconstruction
    )
