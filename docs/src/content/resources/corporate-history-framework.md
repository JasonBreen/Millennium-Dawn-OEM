...
The corporate-history framework centralizes dated company and national-industry chains, value bounds, game-rule gating, catch-up, and silent historical reconstruction. The schema-v6 manifest declares 33 owner chains and four explicitly registered independent subsystems:

- **USA**: Apple, Dell, E3, Google, Texas Instruments, Micron, Motorola, AIG, HP, IBM, Intel, NVIDIA, Oracle, Sun/Microsoft, and Xbox.
...
Schema v6 adds `independent_subsystems[]`. Every entry declares `id`, `kind`, `namespaces`, explicit `event_ids`, `owner_tags`, `reconstruction_effects`, `scheduler_entrypoints`, `effect_roots`, and `mode_policy`. GPU Development, Israel OEM historical flavour, and legacy U.S. OEM/storage use `full_events_outcomes_reconstruct_off_inert`. The Physical Compute Stack uses `derived_only`, has no event IDs or scheduler entrypoint, and is reached only from its declared producer root. Its original TI/Micron/Motorola capstone remains sufficient by itself; Intel can add an optional processor-architecture pillar and a separate full-spectrum enhancement without becoming a fourth requirement. Wildcard event ownership and undeclared on-action dispatch are invalid.

`USA_physical_compute_stack_resolve` owns the optional extension. Any viable Intel terminal idea except `USA_intel_managed_retrenchment` produces `USA_processor_architecture_anchor`. `USA_full_spectrum_compute_stack` then requires both that anchor and the unchanged `USA_physical_compute_stack` idea. Removing either input removes only the corresponding extension; it does not change the original three-pillar threshold or write Intel state.
...
The [OEM upstream packaging plan](/dev-resources/oem-upstream-packaging-plan/) defines the dependency-ordered pull-request series. The current 33-chain implementation is not intended for submission as one monolithic change.
...
| Intel                  | Owns its seven bounded axes, era, delivery lifecycle, and terminal ideas. Apple Intel/silicon transitions, Micron IM Flash depth, NVIDIA CUDA commitment, IBM x86 divestiture, TSMC advanced packaging, ATI/AMD ROCm openness, and VIA cross-licensing are read-only event context; none gates reconstruction or delivery. Shared U.S. systems may read Intel's authoritative capstones; Intel never writes another chain's state.                                                                                                             | `events/USA_intel_events.txt`, `common/scripted_effects/USA_intel_effects.txt`                           |
...
| OEM bridge             | Read-only terminal outcomes of Apple, HP, NVIDIA, Intel, Dell, TI, Micron, Motorola, Google and Oracle, plus the IBM-owned `USA_oem_*` base axes. Writes only its own derived variables and the five `USA_corporate_systems_economic_integration_*` ideas, never chain state. Chain-to-axis table: `.claude/docs/usa-oem-economic-bridge-map.md`                                                                                                                                                                                               | `common/scripted_effects/USA_corporate_systems_effects.txt`                                              |
...
| Intel (USA)                                         | 1                             | 15 visible events, 7 bounded strategy axes plus a bounded era, 6 outcome ideas, owner-local delivery recovery, scheduler + reconstruction                                                                                                                      |
